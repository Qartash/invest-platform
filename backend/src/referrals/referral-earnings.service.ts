...
export class ReferralEarningsService {
  ...
  async qualifyInvitee(invitee: User, triggerTransactionId: string | null): Promise<void> {
    ...
    await this.earningsRepository.save(rows);
  }
  ...
  async payMaturedEarnings(): Promise<void> {
    ...
    for (const earning of due) {
      try {
        if (await this.payOne(earning.id, admin.id)) paid += 1;
      } catch (err) {
  ...
  // beneficiary is gone.
  private async payOne(earningId: string, adminId: string): Promise<boolean> {
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!earning || earning.status !== ReferralEarningStatus.PENDING) return false;
      ...
      if (parseFloat(adminWallet.balance) < amount) {
        return false; // admin out of funds — try again next run
      }
